...
{
    switch (err) {
    case PAPI_UTIL_OK: return "success";
    case PAPI_UTIL_ERRNO: return strerror(errno);
    case PAPI_UTIL_PARSE_ERROR: return "invalid formula";
    case PAPI_UTIL_PAPI_NOT_SUPPORTED: return "PAPI not supported; please rebuild with PAPI support";
    case PAPI_UTIL_PAPI_VERSION_MISMATCH: return "PAPI version mismatch";
#ifdef HAVE_PAPI
    case PAPI_UTIL_PAPI_ERROR: return PAPI_strerror(papierr);
#else
    case PAPI_UTIL_PAPI_ERROR: return "PAPI not supported; please rebuild with PAPI support";
#endif
...
{
    return PAPI_UTIL_PAPI_NOT_SUPPORTED;
}
...
static int get_formula(struct user_formula *f, const char *string)
{
    ...
    f->root = calloc(1, sizeof(struct exptree_node));
    if (!f->root) return PAPI_UTIL_ERRNO;
 ...
 * @return new node
 */
static int _make_node(struct exptree_node *parent, struct exptree_node ** node)
{
    struct exptree_node *self;
    self = calloc(1, sizeof(struct exptree_node));
    if (!self) return PAPI_UTIL_ERRNO;
    self->parent = parent;
    if (parent->left == NULL) {
        parent->left = self;
...
static struct exptree_node *_make_exptree(struct exptree_node *self, char *string)
{
    ...
    // open braces: new node and step to child
        ...
        struct exptree_node *new;
        retval = _make_node(self, &new);
 ...
 */
int read_eventfile(const char *event_file)
{
    // char **events = NULL;
    int num_events = 0;
    int read_formulas = 0;
    ...
    while ((getline(&lineptr, &n, file)) != -1) {
        ...
        // get derived formula from line
            ...
            int retval;
            retval = get_formula(&_formulas[_num_formulas], lineptr);
	    if (retval == PAPI_UTIL_OK) {
                _num_formulas++;
            } else {
                free(lineptr);
                fclose(file);
...
int PAPI_UTIL_setup(const struct papi_util_opt *opt, int *papierr)
{
    if (_initialized) return PAPI_UTIL_OK;
    ...
    int retval;
    retval = read_eventfile(_opt.event_file);
    if (retval) return retval;

    // printf("eventfile: %s\n", event_file);
    retval = PAPI_library_init(PAPI_VER_CURRENT);
    if (retval != PAPI_VER_CURRENT && retval > 0) {
        return PAPI_UTIL_PAPI_VERSION_MISMATCH;
        /* eprintf("PAPI library version mismatch!\n"); */
        /* exit(EXIT_FAILURE); */
    }
    if (retval < 0) {
        if (papierr) *papierr = retval;
        return PAPI_UTIL_PAPI_ERROR;
